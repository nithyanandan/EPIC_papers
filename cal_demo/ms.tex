...
Here we study the properties of our calibration solutions with respect to noise in the system, and compare to solutions found from visibilities. For this comparison we run a suite of simulations varying the receiver noise and integration times, while forming $\Cna$ correlations for EPICal gain solutions, and simultaneously forming visibilities from the same E-field streams to find visibility-based gain solutions.

We use a simulated sky consisting of a 5 Jy calibrator source, and 10 other random sources with apparent flux densities 0.2 to 0.5 Jy (total sky power of about 8 Jy). While we generate these other sources randomly, we keep them fixed for each run. Again, we use the MWA core layout (51 antennas), and a 4.4~m square tophat antenna voltage pattern. We simulate 64 frequency channels. Because our calibration loop is frequency independent, each channel can be treated as a separate trial of the simulation, and is used to better estimate the statistics.
...
We initialize our gain estimates with the true values (unity), and allow the estimate to be corrupted by the noise through one iteration of the calibration loop. In order to allow the noise to fully corrupt the gains we use a small damping factor, $\damp=0.05$. Using a factor of zero leads to non-convergent results. We therefore use the same damping factor for the visibility-based solutions for a fair comparison. We then observe the error in the gain estimates introduced by the noise.
