...
on the ground as the integral of the propagated fields from the sphere.
\begin{equation}
...
aperture.
\begin{equation}
E^I_a(f,t) = \int_{\mathrm{aperture}}W_a(\mathbf{r}-\mathbf{r}_a)E_a(\mathbf{r},t)\,d^2\mathbf{r}
\end{equation}
...
\Ethetaest(\s_i,t) & = \frac{1}{\Nant} \sum_a \frac{g^T_a}{g'_a}\Er{a}^T(t) e^{2\pi i \s_i \cdot \ra} \nonumber \\
  & \qquad\times \sum_i \beamr_a(\ri-\ra)e^{2\pi i \s_i \cdot (\ri-\ra)} \nonumber\\
