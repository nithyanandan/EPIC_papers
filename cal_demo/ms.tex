...
\Cna \equiv \left<E_a^{(n)}(t) \mathcal{E}'^{*(n)}(\spix,t)\right>_t,
\end{equation}
...
\Cna & = \left<\left(g_a E_a^I(t)+n_a(t)\right) \frac{1}{\Nant} \right. \nonumber \\
...
& = \frac{1}{\Nant} \sum_b \frac{1}{g^{*(n)}_b} \mathcal{W}^*_b(\spix) e^{-2\pi i \spix \cdot \rb} \nonumber \\
& \qquad \times \left(g_a g^*_b \left<E^I_a(t)E^{*I}_b(t)\right>_t + \left<n_a(t) n^*_b(t)\right>_t\right) \nonumber \\
...
in the third step we assume the noise does not correlate with the ideal electric field 
measurements, and in the final step we introduce the baseline noise $n_{ab}\equiv
\left<n_a(t)n^*_b(t)\right>_t$, and ideal visibilities as the correlation between ideal
electric field measurements, $V^I_{ab} \equiv\left<E^I_a(t)E^{*I}_b(t)\right>_t$.
To be clear, visibilities are never directly formed from the data. Rather it is convenient to
express $\Cna$ in terms of the ideal visibilities, which we can model when solving for the
gains below.

\textcolor{red}{need a brief comment on the noise term}
