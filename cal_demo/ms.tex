...
imaging involves a spatial fast Fourier transform of antenna \textbf{signals}, alleviating the harsh 
...
cross-multiply the \textbf{signals} from all pairs of antennas, and 
...
traditional \textbf{cross correlator}, pair-wise visibilities are written to disk and used to calibrate 
\textbf{after observing before further processing} such as imaging. However, a direct imaging correlator mixes 
...
\textbf{It is convenient to simplify notation by expressing the position vector in units of wavelengths, 
...
framework. In the limit of a single bright calibrating source \textbf{at center,} we can greatly 
...
box of 150~m. The \textbf{antenna aperture illumination pattern} used is a 4.4~m square tophat; \textbf{a very rough 
...
are used to update the gain estimates. \textbf{For this simple demonstration we create perfect model 
...
initialized randomly, while the true values were unity. After about \textbf{12} iterations we see the 
...
trend. }
The same level of \textbf{gain error as visibility based solutions} can be achieved with a longer integration time (or larger damping 
factor).
...
left panel shows the uncalibrated image integrated over 51.2 ms, \textbf{83 kHz.} Cyg~A is prominent 
...
were produced with \textbf{83 kHz} bandwidth and 51.2 ms integration. The calibrated image shows 
...
absolute deviation of the image, a robust statistic. With this metric we find the \textbf{uncalibrated 
and} calibrated images to have \textbf{dynamic ranges of 88.5 and 137, respectively, a 55\% 
improvement.}
