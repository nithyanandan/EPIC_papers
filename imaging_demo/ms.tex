...
% process in the former involves tying the antenna footprint (either in 
% analytic or lookup table formats) to the nearest grid pixel. Depending on the 
% exact location of the center of the antenna relative to the grid and the 
% coarseness of grid spacing, grid pixels that receive contribution from an 
...
% typically leave some residuals behind as seen in the void region in the weights 
% obtained with MOFF imaging. 

% These residuals can be mitigated by: 
% \begin{enumerate}
%   \item making the grid spacing finer which makes the nearest neighbour 
%     association between the antenna footprint and the grid less susceptible 
%     (proportional to grid spacing) to the location of the antenna center 
%     relative to the grid, and
...
% because with MOFF imaging the antenna locations were tied to the nearest 
% grid pixel whereas in visibility based imaging the baseline locations 
% (difference between antenna locations) were tied to the nearest grid pixel. 
