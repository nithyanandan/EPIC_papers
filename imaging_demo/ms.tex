...
locations around the antennas. A nearest neighbour mapping from all antenna 
...
\begin{figure}
  ...
  \caption{Auto-correlation of weights of a square shaped antenna aperture
    ...
    power pattern on the sky. The colour scale for the directional power 
...
\begin{figure}
  ...
  \caption{Map of difference between the synthesized beams obtained with the 
    ...
    beam (grey) and that of the rms of the difference (black) are shown as 
...
% {\bf In the above example, we chose a scenario wherein the antennas in the array have large differences between them to clearly demonstrate the functionality in EPIC to image data from heterogeneous arrays. The analysis, however, is applicable to any heterogeneous array. In cases where the differences are smaller, the fidelity with which the sky model is recovered will still be degraded, although to a correspondingly smaller extent. To robustly quantify this degradation as a function of deviation from homogeneity will require image deconvolution and will be investigated in the future as it is currently beyond the scope of this paper.}
...
same core diameter. The grey shaded area is for a projected LWA expansion and is 
...
\begin{figure}
  ...
  \caption{Current and planned instruments in parameter space of
    ...
    the same core. The grey shaded area denotes the projected trajectory of the 
