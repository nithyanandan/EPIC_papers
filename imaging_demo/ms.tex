...
($\Nant\gtrsim 1000$--$\gtrsim 10000$). Since the complexity of traditional 
correlators scales as $\mathcal{O}(\Nant^2)$, there will be a steep cost for 
...
only does it bring down the cost to $\mathcal{O}(\Nant\log \Nant)$ 
...
timescales roughly equal to the writeout timescale of the digitizer and vastly 
lower bandwidth relative to visibility based systems will make it a prime 
candidate for transient searches of Fast Radio Bursts (FRB) as well as 
planetary and exoplantenary phenomena. 
\end{abstract}
...
Array (LWA, \citealt{ell13}), and the low frequency component of the Square 
Kilometer Array Low Frequency Aperture Array (SKA1-Low \citealt{mel13}).
...
$\Ngrid\log\Ngrid$, where $\Ngrid$ is the number of grid pixels in the Fourier 
...
$\Ngrid \log \Ngrid$ computational scaling without placing any restriction on
...
code. We begin with a technical description of the algorithm in \S\ref{sec:math}, 
...
image $I(f)$. The $I(f)$ obtained from both techniques are identical as explained 
in \S\ref{sec:math}.
\begin{figure}
  ...
  \caption{A flowchart of visibility based software holographic imaging in EPIC. 
    ...
    F-engine, the electric fields pass through the X-engine to obtain 
    ...
    are then Fourier transformed to obtain the image.
    $I(f)$.}
...
field spectra. This step can be parallelized by antennas as shown in 
...
operation at every timestamp for every frequency channel. In visibility based 
...
sky and process the data through both the MOFF and visibility based imaging 
algorithms. We then compare the output images to demonstrate their equivalence.
...
\begin{figure}
  ...
  \caption{Image from LWA TBN data obtained with MOFF imaging using EPIC 
    ...
    and not calibrated. Locations of Cyg~A and Cas~A are annotated.}
  \label{fig:LWA-image}
\end{figure}
...
that for visibility based imaging comes from $\Nant(\Nant-1)/2$ 
...
$\approx \beta \Ngrid\log \Ngrid$ where $\Ngrid$ 
...
set $\beta=5$, a value\footnote{http://www.fftw.org/speed/method.html} much 
more conservative than was indicated in \citet{mor11}. We set the number of 
complex multiplications in the X-engine in visibility based imaging to 
$\Nant^2$.
...
e.g., MOFF imaging is favoured in HERA-33 and HERA-6769 because they are inside 
...
a fixed baseline length, regions favouring the MOFF algorithm tend to be towards 
...
\begin{figure}
  ...
  \caption{Current and planned instruments in parameter space of
    ...
    based imaging is favoured for the corresponding antenna size. Region inside 
    the wedge enclosed by the solid and dashed lines favours using the MOFF 
    ...
    is favoured in HERA-33 and HERA-6769 because they are inside the wedge 
...
second) with MOFF and 
X-engine outputs are: 
\begin{align}
...
\end{align}
where, factor 4 in the expression for $r_\textrm{MOFF}$ accounts for imaging
...
$\Ngrid \simeq 1024$ if $\Ngrid$ is preferred as a power of 2. 
Using 8 bytes for each floating point sample, the throughputs are 
$r_\textrm{MOFF} \lesssim 3$~GB/s and $t_\textrm{X} \simeq 81$~GB/s. For HERA-37,
$r_\textrm{MOFF} \lesssim 190$~MB/s and $t_\textrm{X} \simeq 1$~GB/s. In such a 
case, The X-engine throughput corresponds to $\simeq 3.5$~TB an hour. 
Conversely, for the same data throughput, the MOFF algorithm can sample even 
shorter timescales. Table~\ref{tab:data-rates} shows these data rates for some 
of the current and planned telescopes for $\Delta t=10$~ms. 

\begin{table}
  \centering
  \caption{Data throughput for various telescopes with MOFF and X-engine 
    outputs on timescales of $\Delta t=10$~ms assuming $\Delta B=100$~MHz and
    $\Delta f=100$~kHz.}
  ...
  \begin{tabular}{ccc} 
    ...
    Telescope\tnote{a} & $r_\textrm{MOFF}$\tnote{b} & $r_\textrm{X}$ \\
              & (GB/s) & (GB/s) \\
    \hline
    LWA1 & $\simeq 3$ & $\simeq 48.5$ \\
    LWA-OV & $\simeq 12$ & $\simeq 48.5$ \\
    HERA-19 & $\lesssim 0.19$ & $\simeq 0.25$ \\
    HERA-37 & $\lesssim 0.19$ & $\simeq 1$ \\
    HERA-331 & $\lesssim 3$ & $\simeq 81$ \\
    CHIME & $\lesssim 6.1$ & $\simeq 1220$ \\
    \hline
  \end{tabular}
  \begin{tablenotes}
    ...
    \item[b] $\Ngrid$ is usually greater than true value because of 
      rounding to the next power of 2. Thus $r_\textrm{MOFF}$ estimates are
      usually lesser than the conservative values listed here.
  \end{tablenotes}
  \end{threeparttable}
\end{table}

Thus even with conservative estimates, the MOFF algorithm provides very 
economic data throughput for a majority of next generation radio telescopes
with a dense layout. The most significant advantage is that calibrated images 
...
integrated module with significant savings in data throughput relative to a 
X-engine based pipeline. Hence, it is a compelling candidate for time-domain 
radio astronomy, e.g. search for and monitoring of transients. Transient 
detection pipeline at the backend of EPIC can be fine-tuned to target fast 
transients such as the Fast Radio Bursts \citep[FRB;][]{tho13} on millisecond 
timescales at GHz frequencies or slow transients from planetary and 
